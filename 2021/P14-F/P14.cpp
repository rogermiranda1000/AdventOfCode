...
std::pair<int, int> getLeastAndMostCommonAppearances(std::vector<char> result) {
	std::map<char,int> apperances;
	...
	std::pair<int,int> min_max = std::make_pair(0, 0);
...
int main() {
	...
	std::pair<int, int> solution = getLeastAndMostCommonAppearances(result);
